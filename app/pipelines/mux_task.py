...
def handle_mux_task(job_details: dict):
    ...
    try:
        ...
        speakers_list = []  # 추가: speakers 리스트 형식
                ...
                for speaker, ref_data in speaker_refs_mapping.items():
                    if isinstance(ref_data, dict):
                        ...
                        if audio_path:
                            ...
                            # speakers 리스트 형식으로도 추가
                            speakers_list.append(
                                {
                                    "speaker": speaker,
                                    "voice_sample_key": ref_s3_key,  # s3:// 접두사 제거
                                    "prompt_text": prompt_text,
                                }
                            )
        ...
        # manifest에서 voice_replacements 정보 읽기
        voice_replacement_meta = {}
        if "voice_replacements" in manifest:
            ...
            # voice_replacements에서 voice_sample_id 추출하여 speakers_list에 추가
            for speaker, replacement_info in voice_replacements.items():
                if isinstance(replacement_info, dict):
                    ...
                    # speakers_list에서 해당 speaker 찾아서 voice_replacement 정보 추가
                    for speaker_entry in speakers_list:
                        if speaker_entry["speaker"] == speaker:
                            speaker_entry["voice_replacement"] = {
                                "voice_sample_id": voice_id,
                                "similarity": similarity,
                                "sample_key": sample_key,
                            }
                            break
        elif "voice_replacement_diagnostics" in manifest:
            voice_replacement_meta = manifest["voice_replacement_diagnostics"]
        else:
            voice_replacement_meta = {
                "enabled": False,
        ...
        # 추가: speakers 리스트와 voice_replacement 메타데이터
        if speakers_list:
            final_metadata["speakers"] = speakers_list
            final_metadata["speaker_count"] = len(speakers_list)
